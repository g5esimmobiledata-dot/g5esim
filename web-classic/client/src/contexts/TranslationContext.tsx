...
import englishTranslations from "@/locales/en_translations.json";
import arabicTranslations from "@/locales/ar_translations.json";
import frenchTranslations from "@/locales/fr_translations.json";
import spanishTranslations from "@/locales/es_translations.json";
import russianTranslations from "@/locales/ru_translations.json";
import italianTranslations from "@/locales/it_translations.json";
...
const BUILT_IN_TRANSLATIONS: Record<string, TranslationData> = {
  en: englishTranslations as TranslationData,
  ar: arabicTranslations as TranslationData,
  fr: frenchTranslations as TranslationData,
  es: spanishTranslations as TranslationData,
  ru: russianTranslations as TranslationData,
  it: italianTranslations as TranslationData,
};
...
function buildPhraseMap(languageCode: string, translations: TranslationData) {
  const map = new Map<string, string>();
  if (languageCode === "en" || !BUILT_IN_TRANSLATIONS[languageCode]) return map;

  const englishFlat = flattenTranslations(englishTranslations as TranslationData);
  const builtInFlat = flattenTranslations(BUILT_IN_TRANSLATIONS[languageCode]);
  const activeFlat = flattenTranslations(mergeTranslationData(BUILT_IN_TRANSLATIONS[languageCode], translations));
...
export function TranslationProvider({ children }: { children: ReactNode }) {
  ...
  const phraseMap = useMemo(
    () => buildPhraseMap(languageCode, translations),
  ...
  const t = useCallback(
    (
      ...
      if (translationKey) {
        const builtInValue = BUILT_IN_TRANSLATIONS[languageCode]?.[namespace]?.[translationKey];
        const englishValue = BUILT_IN_TRANSLATIONS.en?.[namespace]?.[translationKey];
    ...
    [translations, languageCode, phraseMap]
  );
...
export function useTranslation() {
  ...
  const fallbackT: TranslationContextType["t"] = (key, fallbackOrParams, params) => {
    ...
    const namespace = keyParts[0];
    const translationKey = keyParts.slice(1).join(".");
    const builtInValue = BUILT_IN_TRANSLATIONS[fallbackLanguageCode]?.[namespace]?.[translationKey];
    const fallback = typeof fallbackOrParams === "string" ? fallbackOrParams : undefined;
    const actualParams = typeof fallbackOrParams === "string" ? params : fallbackOrParams;
    const phraseMap = buildPhraseMap(fallbackLanguageCode, {});
    const rawValue = builtInValue || fallback || key;
    const value =
      fallbackLanguageCode === "en"
        ? normalizeDisplayWords(rawValue)
        : translateLiteral(rawValue, phraseMap);
