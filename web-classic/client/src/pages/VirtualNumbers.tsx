...
export default function VirtualNumbers() {
  ...
  const [requestForwardingType, setRequestForwardingType] = useState<'none' | 'international' | 'sip' | 'voicemail'>('sip');
  ...
  const { data: selection, isLoading: isSelectionLoading } = useQuery<VirtualNumberSelection>({
    ...
    queryFn: async () => {
      ...
      if (!json.success) throw new Error(json.message || 'Could not load available DID numbers');
  ...
  const applyMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      setRequestForwardingDestination(userSipAccount?.uri || '');
    },
    onError: (error: any) => {
      toast({
  ...
  const useUserSipForRequest = () => {
    if (!userSipAccount?.uri) return;
    setRequestForwardingType('sip');
    setRequestForwardingDestination(userSipAccount.uri);
  };

  const useUserSipForNumber = (
    numberId: string,
    localSettings: {
      autoRenew: boolean;
      reminderDays: string;
    ...
    setNumberSettings((current) => ({
      ...
      [numberId]: {
        ...localSettings,
        forwardingType: 'sip',
        forwardingDestination: userSipAccount.uri,
      },
    }));
  };

  useEffect(() => {
    if (requestForwardingType !== 'sip' || !userSipAccount?.uri) return;
    setRequestForwardingDestination((current) => current || userSipAccount.uri);
  }, [requestForwardingType, userSipAccount?.uri]);
          ...
          <CardContent className="grid gap-4 p-5 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-semibold">My Free SIP Account</h2>
              ...
              <p className="mt-3 text-xs text-cyan-50/65">
                Register this account in Linphone or Zoiper, then forward your eRoaming Number to it to receive calls.
              </p>
        ...
        <Card className="overflow-hidden border-blue-300/10 bg-blue-950/25 text-white shadow-xl shadow-black/30 backdrop-blur">
          ...
          <CardContent className="grid gap-6 p-0 lg:grid-cols-[280px_1fr]">
            ...
            <div className="space-y-2">
              <Label htmlFor="virtual-country">DID Country</Label>
              <select
                id="virtual-country"
            ...
            <div className="space-y-2 md:col-span-2">
              <Label htmlFor="available-did">Available DID Virtual Number</Label>
              <select
                id="available-did"
                className="flex h-10 w-full rounded-md border border-blue-300/15 bg-slate-950/70 px-3 py-2 text-sm text-white"
              ...
              >
                <option value="">
                  {isSelectionLoading
                    ? 'Loading available DID numbers...'
                    : availableNumbers.length
                      ? 'Auto assign or manual review'
                      : 'No saved DID inventory for this country'}
                </option>
              ...
              {selectedInventory && (
                <div className="rounded-lg border border-blue-300/10 bg-slate-950/45 p-3 text-xs text-cyan-50/80">
                  Selected DID: {selectedInventory.msisdn} - Setup ${selectedInventory.setupFee}, monthly ${selectedInventory.monthlyFee}, inbound ${selectedInventory.inboundFee}, outbound ${selectedInventory.outboundFee}.
                </div>
              )}
            </div>
            <div className="space-y-2">
              ...
              />
              {selectedInventoryId && <p className="text-xs text-cyan-50/65">Exact DID selection supports one number per request.</p>}
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => {
                  const value = e.target.value as 'none' | 'international' | 'sip' | 'voicemail';
                  setRequestForwardingType(value);
                  if (value === 'sip' && userSipAccount?.uri) {
                    setRequestForwardingDestination(userSipAccount.uri);
                  }
                  if (value === 'none' || value === 'voicemail') {
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={userSipAccount?.uri || '+12025550199 / sip:user@example.com'}
                className="border-blue-300/15 bg-slate-950/70 text-white placeholder:text-slate-500"
              />
            </div>
      ...
      {data?.application && !data.number && (
        <Card className="border-blue-300/10 bg-blue-950/25 text-white shadow-xl shadow-black/30 backdrop-blur">
          ...
          <CardContent className="grid gap-3 text-sm md:grid-cols-2">
            ...
            <div><span className="font-medium">Forwarding:</span> {data.application.metadata?.forwardingType && data.application.metadata.forwardingType !== 'none' ? `${data.application.metadata.forwardingType}${data.application.metadata.forwardingDestination ? ` • ${data.application.metadata.forwardingDestination}` : ''}` : 'None'}</div>
        ...
        <>
            ...
            <CardContent className="space-y-4">
              {numbers.map((number) => {
                ...
                const localSettings = numberSettings[number.id] || {
                  ...
                  forwardingType: (routing?.type as any) || (userSipAccount?.uri ? 'sip' : 'none'),
                  forwardingDestination: routing?.destination || (userSipAccount?.uri ? userSipAccount.uri : ''),
                  ...
                  <div key={number.id} className="rounded-2xl border border-blue-300/10 bg-slate-950/35 p-4">
                    ...
                    <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                      ...
                      <div className="rounded-xl border border-blue-300/10 bg-blue-400/10 p-4">
                        ...
                        <div className="mt-2 text-sm font-semibold">{routing?.type && routing.type !== 'none' ? `${routing.type}${routing.destination ? ` • ${routing.destination}` : ''}` : 'None'}</div>
                      </div>
                      <div className="rounded-xl border border-blue-300/10 bg-blue-400/10 p-4">
                    ...
                    <div className="mt-4 rounded-xl border border-blue-300/10 bg-slate-950/45 p-4">
                      ...
                      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <Input className="border-blue-300/15 bg-slate-950/70 text-white" value={localSettings.reminderDays} onChange={(e) => setNumberSettings((current) => ({ ...current, [number.id]: { ...localSettings, reminderDays: e.target.value } }))} />
                        </div>
                        <div className="space-y-2">
                              ...
                              setNumberSettings((current) => ({
                                ...
                                [number.id]: {
                                  ...
                                  forwardingDestination:
                                    value === 'sip'
                                      ? userSipAccount?.uri || localSettings.forwardingDestination
                                      : value === 'none' || value === 'voicemail'
                                        ? ''
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {userSipAccount?.uri && (
                              <Button
                                ...
                                size="sm"
                                className="h-8 border-cyan-300/30 bg-cyan-300/10 text-cyan-50 hover:bg-cyan-300/20"
                                onClick={() => useUserSipForNumber(number.id, localSettings)}
                              >
                                My SIP
                              </Button>
                            )}
                          </div>
                          <Input className="border-blue-300/15 bg-slate-950/70 text-white placeholder:text-slate-500" value={localSettings.forwardingDestination} onChange={(e) => setNumberSettings((current) => ({ ...current, [number.id]: { ...localSettings, forwardingDestination: e.target.value } }))} placeholder={userSipAccount?.uri || '+12025550199 / sip:user@example.com'} />
                        </div>
                        <div className="space-y-2">
          ...
          <Card className="border-blue-300/10 bg-blue-950/25 text-white shadow-xl shadow-black/30 backdrop-blur">
            <CardHeader>
              ...
              <CardDescription className="text-cyan-50/75">Start an eRoaming voice session from your active DID number</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-5 lg:grid-cols-[1fr_260px]">
              ...
              <div className="rounded-2xl border border-blue-300/10 bg-slate-950/45 p-4">
                ...
                <div className="mt-2 text-lg font-semibold">{primaryNumber?.msisdn || 'No active DID'}</div>
