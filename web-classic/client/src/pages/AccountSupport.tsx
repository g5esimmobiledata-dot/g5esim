...
export default function AccountSupport() {
    ...
    settings?.concierge_hotline_enabled !== 'false'
      ? settings?.concierge_hotline_url ||
        (settings?.concierge_hotline_number
          ...
          : '')
      : '';
  const sipPublicEnabled = settings?.concierge_sip_enabled === 'true' || Boolean(conciergeSip?.enabled);
  ...
  return (
    <div className="space-y-8">
      ...
      {whatsappUrl && (
        <Card className="border-green-400/20 bg-gradient-to-r from-emerald-400/10 to-lime-300/10 text-white shadow-xl shadow-black/10">
          <CardContent className="flex flex-col gap-4 p-6 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-3">
      ...
      {sipPublicEnabled && (
        <Card className="border-sky-400/20 bg-gradient-to-r from-sky-400/10 to-cyan-300/10 text-white shadow-xl shadow-black/10">
          <CardContent className="flex flex-col gap-4 p-6 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-white">Call Center</h2>
                <p className="mt-1 text-sm text-slate-300">
                  {vipConciergeLocked
                    ...
                    : sipCalling
                      ...
                      : 'SIP calling is enabled but the call address is not configured yet.'}
                </p>
                {vipConciergeLocked && (
                  <p className="mt-2 text-xs font-medium uppercase tracking-wide text-amber-200">
                    VIP Concierge payment required
