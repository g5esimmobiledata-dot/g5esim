...
import { generateInstallationEmail, sendEmail } from 'server/email';
...
async function sendInstallationEmailForOrder(order: any, metadata: Record<string, any>) {
  ...
  try {
    const installEmail = await generateInstallationEmail({
      name: customer?.name || metadata.name || 'Customer',
      packageName: `${normalizedOrder.dataAmount} - ${normalizedOrder.validity} Days`,
