...
export async function ensureSipCommercialSchema() {
  if (!ensurePromise) {
    ensurePromise = (async () => {
      ...
      `);
      await db.execute(sql`CREATE INDEX IF NOT EXISTS sip_tariffs_type_idx ON sip_tariffs(tariff_type)`);
      await db.execute(sql`CREATE INDEX IF NOT EXISTS sip_tariffs_status_idx ON sip_tariffs(status)`);
      ...
      `);
      await db.execute(sql`CREATE INDEX IF NOT EXISTS sip_rate_groups_status_idx ON sip_rate_groups(status)`);
      ...
      `);
      await db.execute(sql`CREATE INDEX IF NOT EXISTS sip_registration_profiles_status_idx ON sip_registration_profiles(status)`);
      await db.execute(sql`CREATE INDEX IF NOT EXISTS sip_registration_profiles_is_default_idx ON sip_registration_profiles(is_default)`);
    })();
  }
...
export async function getSipRegistrationProfileById(profileId?: string | null) {
  ...
  if (!id) return null;
  const [profile] = await db.select().from(sipRegistrationProfiles).where(eq(sipRegistrationProfiles.id, id)).limit(1);
  return profile || null;
}
...
export function mergeSipFeatureSettings(
  ...
  return {
    ...profileMetadata,
    ...overrides,
    registrationProfileId: overrides.registrationProfileId ?? profile?.id ?? profileMetadata.registrationProfileId ?? null,
    registrationProfileName: overrides.registrationProfileName ?? profile?.name ?? profileMetadata.registrationProfileName ?? "",
  };
}
