...
const DESKTOP_ADMIN_LOGIN_STORAGE_KEY = 'g5DesktopAdminSavedLogin';
...
export default function AdminLogin() {
  ...
  useEffect(() => {
    const desktopBridge = (window as typeof window & { g5Desktop?: DesktopBridge }).g5Desktop;
    const desktopParam = new URLSearchParams(window.location.search).get('desktop') === '1';
    const runningInDesktop = Boolean(desktopBridge) || desktopParam;
    setIsDesktopMode(runningInDesktop);

    if (!runningInDesktop) return;

    try {
      const savedLogin = JSON.parse(localStorage.getItem(DESKTOP_ADMIN_LOGIN_STORAGE_KEY) || '{}');
      if (savedLogin?.email) setEmail(savedLogin.email);
      if (savedLogin?.password) setPassword(savedLogin.password);
      setSaveLogin(Boolean(savedLogin?.email || savedLogin?.password));
    } catch {
      localStorage.removeItem(DESKTOP_ADMIN_LOGIN_STORAGE_KEY);
    }
  }, []);
      ...
      if (isDesktopMode) {
        if (saveLogin) {
          localStorage.setItem(
            DESKTOP_ADMIN_LOGIN_STORAGE_KEY,
            JSON.stringify({
              email,
              password,
  ...
  const authFontStyle = { fontFamily: "'Poppins', 'Inter', 'Segoe UI', Arial, sans-serif" };

  return (
    <div className="didww-auth-page flex min-h-screen bg-white text-slate-950" style={authFontStyle}>
      <SEOHead
      ...
      <div className="didww-auth-brand relative hidden flex-1 overflow-hidden p-12 lg:flex lg:flex-col lg:items-center lg:justify-center">
        ...
        <p
          className="absolute bottom-8 left-0 right-0 text-center text-sm font-medium text-white/80"
          style={authFontStyle}
        >
          Copyright 2026 G5eSIM Mobile. All Rights Reserved.
        </p>
      </div>
              ...
              <form onSubmit={handleLogin} className="didww-admin-login-form space-y-6 bg-white p-0 shadow-none">
                ...
                {isDesktopMode && (
                  <label className="flex cursor-pointer items-center gap-3 text-sm font-medium text-slate-600">
                    <Checkbox
                      ...
                      disabled={isLoading}
                      className="h-5 w-5 rounded border-slate-300 data-[state=checked]:bg-[#4f73bd] data-[state=checked]:text-white"
                      data-testid="checkbox-save-admin-login"
                    />
                ...
                {isDesktopMode && (
                  <Button
                    ...
                    variant="outline"
                    onClick={handleExit}
                    className="h-14 w-full rounded-full border-slate-300 bg-white text-base font-semibold text-slate-700 shadow-sm hover:bg-slate-50"
                    disabled={isLoading}
                    data-testid="button-desktop-exit"
                  >
