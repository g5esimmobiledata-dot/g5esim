...
export default function MyOrders() {
  ...
  const [installationQrUrl, setInstallationQrUrl] = useState<string | null>(null);
  const [installationDirectQrFailed, setInstallationDirectQrFailed] = useState(false);
  ...
  const getQrImage = async (order: OrderWithDetails) => {
    ...
    return (
      (await toQrImageUrl(orderData.qrCodeImage)) ||
      (await toQrImageUrl(orderData.qr_code_image)) ||
  ...
  const installationManualCode =
    instructions?.manual_code ||
    (selectedOrder as any)?.manualCode ||
    (selectedOrder as any)?.manual_code ||
    selectedOrder?.lpaCode ||
    (!isImageQrValue(selectedOrder?.qrCode) ? selectedOrder?.qrCode : null) ||
    (selectedOrder?.smdpAddress && selectedOrder?.activationCode
      ? `LPA:1$${selectedOrder.smdpAddress}$${selectedOrder.activationCode}`
      : null) ||
    instructions?.activation_code ||
    selectedOrder?.activationCode;
  const directInstallationQrUrl =
    selectedOrder && !installationDirectQrFailed
      ...
      : null;
  const displayedInstallationQrUrl = installationQrUrl || directInstallationQrUrl;

  useEffect(() => {
    let cancelled = false;

    async function resolveInstallationQr() {
      if (!dialogOpen || !selectedOrder) {
        setInstallationQrUrl(null);
        return;
      }

      const qr =
        (await toQrImageUrl((selectedOrder as any).qrCodeImage)) ||
        (await toQrImageUrl((selectedOrder as any).qr_code_image)) ||
        (await toQrImageUrl(instructions?.manual_code)) ||
        (await toQrImageUrl(selectedOrder.lpaCode)) ||
        (await toQrImageUrl(instructions?.qr_code)) ||
        (await toQrImageUrl(brandedQr?.qr_code)) ||
        (await toQrImageUrl(selectedOrder.qrCode)) ||
        (await toQrImageUrl(instructions?.activation_code)) ||
        (await toQrImageUrl(selectedOrder.qrCodeUrl));

      if (!cancelled) {
        setInstallationQrUrl(qr);
      }
    }

    resolveInstallationQr().catch((error) => {
      console.error('Failed to resolve installation QR:', error);
      if (!cancelled) {
        setInstallationQrUrl(null);
      }
    });
  ...
  }, [dialogOpen, selectedOrder, instructions, brandedQr]);

  useEffect(() => {
    setInstallationDirectQrFailed(false);
                ...
                <>
                  <Card>
                    ...
                    <CardContent className="flex flex-col items-center gap-4">
                      {displayedInstallationQrUrl ? (
                        <div className="p-4 bg-white rounded-lg">
                          <img
                            src={displayedInstallationQrUrl}
                            alt="eSIM QR Code"
                            className="w-64 h-64"
                            data-testid="img-qr-code"
                            onError={() => {
                              if (installationQrUrl) {
                                setInstallationQrUrl(null);
                              } else {
                                setInstallationDirectQrFailed(true);
                              }
                            }}
                          />
                          {brandedQr?.qr_code && (
                      ...
                      {installationManualCode && (
                        <div className="w-full space-y-2">
                          <p className="text-sm font-medium">Manual Activation Code:</p>
                          <div className="flex items-center gap-2">
                            <code
                              className="flex-1 rounded-md border border-slate-200 bg-white p-3 text-sm font-mono text-slate-950 shadow-sm break-all dark:border-slate-200 dark:bg-white dark:text-slate-950"
                            ...
                            <Button
                              ...
                              size="icon"
                              onClick={() =>
                                copyToClipboard(installationManualCode, 'manual_code')
                              }
