...
async function processIptvExpiryPushReminders() {
  ...
  for (const order of result.rows) {
    ...
    const oneSignalResult = await sendOneSignalPushToUsers([order.userId], title, message, data).catch((error) => {
      console.warn("OneSignal IPTV expiry push unavailable:", error instanceof Error ? error.message : error);
      return null;
    });

    if (!oneSignalResult?.sent && tokens.length) {
