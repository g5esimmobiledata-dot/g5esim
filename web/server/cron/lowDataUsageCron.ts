            ...
            for (const order of orders) {
                try {
                    ...
                    if (
                        ...
                        const title = "Low Data Alert";
                        const body = `You've used ${crossedLevel}% of your eSIM data. Top up to stay connected.`;
                        const data = {
                            type: "low_data",
                            level: crossedLevel.toString(),
                            iccid,
                        };
                        const oneSignalResult = await sendOneSignalPushToUsers([user.id], title, body, data).catch((error) => {
                            console.warn("OneSignal low data push unavailable:", error instanceof Error ? error.message : error);
                            return null;
                        });
                        let pushSent = Number(oneSignalResult?.sent || 0) > 0;
