...
function isImageQrSource(value?: string | null) {
...
function buildLpaCode(smdpAddress?: string | null, activationCode?: string | null) {
  if (!smdpAddress || !activationCode) return undefined;
  return `LPA:1$${smdpAddress}$${activationCode}`;
}

async function normalizeQrSource(value?: string | null) {
  const text = String(value || '').trim();
  if (!text) return undefined;
  if (isImageQrSource(text)) return text;
  return generateQRCodeDataUrl(text);
}

function firstImageSource(...values: unknown[]) {
  return firstString(...values.filter((value) => isImageQrSource(value as string | null)));
}

function firstQrPayload(...values: unknown[]) {
  return firstString(...values.filter((value) => {
    const text = typeof value === 'string' ? value.trim() : '';
    return Boolean(text) && !isImageQrSource(text);
  }));
}

async function normalizeInstallationInstructions(payload: any, order?: any) {
  const instructions = payload && typeof payload === 'object' ? payload : {};
  const smdpAddress = firstString(
  ...
  const builtLpaCode = buildLpaCode(smdpAddress, activationCode);
  const manualCode = firstString(
    instructions.manual_code,
    instructions.manualCode,
    instructions.lpa_code,
    instructions.lpaCode,
    instructions.lpa?.startsWith?.('LPA:') ? instructions.lpa : undefined,
    order?.lpaCode,
    order?.qrCode && !isImageQrSource(order.qrCode) ? order.qrCode : undefined,
    builtLpaCode,
    activationCode,
  );
  const qrCodeUrl = firstString(
    instructions.qr_code_url,
    instructions.qrCodeUrl,
    instructions.qrcode_url,
    order?.qrCodeUrl,
  );
  const qrPayload = firstQrPayload(
    manualCode?.startsWith('LPA:') ? manualCode : undefined,
  ...
  const qrImage = firstImageSource(
    instructions.qr_code,
    instructions.qrCode,
    instructions.qrcode,
    qrCodeUrl,
  ...
  const qrSource = qrPayload || qrImage;

  return {
    ...instructions,
    qr_code: await normalizeQrSource(qrSource),
    qr_code_url: qrCodeUrl,
    manual_code: manualCode,
    smdp_address: smdpAddress,
    activation_code: activationCode,
    apn_type: instructions.apn_type ?? order?.apnType,
    apn_value: instructions.apn_value ?? order?.apnValue,
    is_roaming: instructions.is_roaming ?? order?.isRoaming,
    direct_apple_url: instructions.direct_apple_url ?? order?.directAppleUrl,
  ...
  async getInstallationInstructions(
    ...
    try {
      ...
      if (order && (order.qrCodeUrl || order.qrCode || order.lpaCode || order.activationCode)) {
        ...
        return normalizeInstallationInstructions({}, order);
      }
      ...
      return normalizeInstallationInstructions(response.data || response, order);
    } catch (error: any) {
