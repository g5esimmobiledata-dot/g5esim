...
async function sendPushToConfiguredProvider(
  recipientIds: string[],
  ...
  const oneSignalResult = await sendOneSignalPushToUsers(recipientIds, title, body, data);
  if (oneSignalResult) {
    return {
      attempted: oneSignalResult.attempted,
      sent: oneSignalResult.sent,
      failed: oneSignalResult.failed,
      errors: oneSignalResult.errors.map((error) => ({
        code: error.code,
...
export async function sendPushCampaign(input: SendPushCampaignInput) {
  ...
  const pushResult = sendPush
    ? await sendPushToConfiguredProvider(recipientIds, tokens, title, message, {
        type: "admin_push",
        title,
        body: message,
        channelId: MOBILE_PUSH_CHANNEL_ID,
  ...
  await pool.query(
    ...
    [
      ...
      JSON.stringify({
        ...
        pushProvider: "provider" in pushResult ? pushResult.provider : "none",
