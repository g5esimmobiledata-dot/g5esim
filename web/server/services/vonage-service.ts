...
import {
  assertOwnSmsCarrierConfigured,
  sendSmsWithOwnCarrier,
} from 'server/services/sms-carrier-service';
import { queueAdminAlert } from 'server/services/admin-alert-service';
import {
  assertSenderIdLimitAvailable,
  getSenderIdLimitStatus,
} from 'server/services/sender-id-limit-service';
import {
  users,
...
function normalizeSmsAddressForLookup(value: unknown) {
  const digits = trim(value).replace(/\D/g, '');
  return digits.startsWith('00') ? digits.slice(2) : digits;
}

function getInboundSmsMessageId(payload: Record<string, any>) {
  return trim(
    payload.messageId ||
      payload.message_id ||
      payload['message-id'] ||
...
function normalizeOutboundVoiceDestination(value: unknown, countryCode?: string | null) {
  ...
  if (/^[1-9]\d{7,14}$/.test(rawDigits)) return rawDigits;
...
function normalizeSmsSenderId(value: unknown) {
  const raw = trim(value);
  if (!raw) return '';

  const phoneLike = raw.replace(/[^\d+]/g, '');
  if (/^\+\d{7,15}$/.test(phoneLike)) return phoneLike.slice(1);
  if (/^00\d{7,15}$/.test(phoneLike)) return phoneLike.slice(2);
  if (/^\d{7,15}$/.test(phoneLike)) return phoneLike;

  const name = raw.replace(/\s+/g, '');
  if (/^[A-Za-z][A-Za-z0-9]{2,10}$/.test(name)) return name;

  throw new Error(
    'Sender ID must be 3-11 letters/numbers starting with a letter, or a 7-15 digit number.',
  );
}

function getSmsSenderIdMetadata(metadata: Record<string, any> | null | undefined) {
  const senderId = ((metadata || {}).senderId || {}) as Record<string, any>;
  const approvedList = Array.isArray(senderId.approvedList)
    ? senderId.approvedList.map((item: unknown) => trim(item)).filter(Boolean)
    : [];
  const approved = trim(senderId.approved) || approvedList[approvedList.length - 1] || null;
  const approvedValues = [...approvedList, approved].filter((item): item is string => Boolean(item));
  const uniqueApprovedList = Array.from(
    new Map(approvedValues.map((item) => [item.toLowerCase(), item])).values(),
  );
  const status = trim(senderId.status) || (approved ? 'approved' : 'none');
  const requested =
    status === 'pending' || status === 'rejected'
      ? trim(senderId.requested) || null
      : null;

  return {
    requested,
    approved,
    approvedList: uniqueApprovedList,
    status,
    requestedAt: trim(senderId.requestedAt) || null,
    reviewedAt: trim(senderId.reviewedAt) || null,
    rejectedReason: trim(senderId.rejectedReason) || null,
  };
}

function toPublicVirtualSmsMessage(message: typeof virtualSmsMessages.$inferSelect) {
  return {
    id: message.id,
...
function toPublicVirtualVoiceCall(call: typeof virtualVoiceCalls.$inferSelect) {
  return {
    id: call.id,
    virtualNumberId: call.virtualNumberId,
...
function toPublicVirtualVoicemail(voicemail: typeof virtualVoicemails.$inferSelect) {
  return {
    id: voicemail.id,
    virtualNumberId: voicemail.virtualNumberId,
...
function toPublicVirtualNumberApplication(application: typeof virtualNumberApplications.$inferSelect | null) {
  if (!application) return null;
  const metadata = (application.metadata as Record<string, any>) || {};
  return {
    id: application.id,
    countryCode: application.countryCode,
    desiredNumber: application.desiredNumber,
    notes: application.notes,
    status: application.status,
    metadata: {
      selectedMsisdn: metadata.selectedMsisdn || null,
      requestQuantity: metadata.requestQuantity || null,
      packageTerm: metadata.packageTerm || null,
      paymentMethod: metadata.paymentMethod || null,
      autoRenew: metadata.autoRenew,
      reminderDays: metadata.reminderDays || null,
      forwardingType: metadata.forwardingType || null,
      forwardingDestination: metadata.forwardingDestination || null,
    },
...
function getApprovedSmsSenderId(metadata: Record<string, any> | null | undefined) {
  return getSmsSenderIdMetadata(metadata).approved || '';
}

function getApprovedSmsSenderIds(metadata: Record<string, any> | null | undefined) {
  const senderId = getSmsSenderIdMetadata(metadata);
  return senderId.approvedList.length ? senderId.approvedList : senderId.approved ? [senderId.approved] : [];
}

function addSmsSenderOption(
  optionMap: Map<string, { senderId: string; label: string; type: string; virtualNumberId?: string; msisdn?: string }>,
  senderId: unknown,
  options: { label?: string; type: string; virtualNumberId?: string; msisdn?: string },
) {
  const sender = trim(senderId);
  if (!sender) return;
  optionMap.set(sender.toLowerCase(), {
    senderId: sender,
    label: options.label || sender,
    type: options.type,
    virtualNumberId: options.virtualNumberId,
    msisdn: options.msisdn,
  });
}

function getSmsSenderOptionsForNumbers(
  numbers: Array<typeof userVirtualNumbers.$inferSelect>,
  selectedNumber?: typeof userVirtualNumbers.$inferSelect | null,
) {
  const optionMap = new Map<
    string,
    { senderId: string; label: string; type: string; virtualNumberId?: string; msisdn?: string }
  >();

  if (selectedNumber) {
    addSmsSenderOption(optionMap, selectedNumber.msisdn, {
  ...
  numbers.forEach((number) => {
    const metadata = (number.metadata as Record<string, any>) || {};
    getApprovedSmsSenderIds(metadata).forEach((sender) => {
      addSmsSenderOption(optionMap, sender, {
        label: sender === number.msisdn ? `${sender} (eRoaming Number)` : sender,
        type: 'approved',
        virtualNumberId: number.id,
        msisdn: number.msisdn,
      });
...
async function resolveSmsSenderForUser(
  userId: string,
  number: typeof userVirtualNumbers.$inferSelect,
  requestedSenderId?: string | null,
) {
  const requested = trim(requestedSenderId);
  if (!requested) {
    const numberMetadata = (number.metadata as Record<string, any>) || {};
    return getApprovedSmsSenderId(numberMetadata) || number.msisdn;
  }

  const normalizedSender = normalizeSmsSenderId(requested);
  if (normalizedSender.toLowerCase() === normalizeSmsSenderId(number.msisdn).toLowerCase()) {
    return number.msisdn;
  }

  const numbers = await getUserVirtualNumbers(userId);
  const approvedOptions = getSmsSenderOptionsForNumbers(numbers, number);
  const selectedOption = approvedOptions.find(
    (option) =>
      option.type === 'approved' &&
      normalizeSmsSenderId(option.senderId).toLowerCase() === normalizedSender.toLowerCase(),
  );

  if (!selectedOption) {
    throw new Error('Select an approved Sender ID before sending SMS.');
  }

  return selectedOption.senderId;
}

type VirtualNumberForwardingType = 'none' | 'international' | 'sip' | 'voicemail';

function normalizeForwardingType(value: unknown, fallback: VirtualNumberForwardingType = 'none') {
  const raw = trim(value).toLowerCase();
  if (raw === 'international' || raw === 'sip' || raw === 'voicemail' || raw === 'none') {
    return raw as VirtualNumberForwardingType;
  }
  return fallback;
}

function normalizeSipDomain(value: unknown) {
  const raw = trim(value).replace(/^sips?:/i, '');
  const withoutUser = raw.includes('@') ? raw.split('@').pop() || raw : raw;
  return withoutUser.replace(/^\/+/, '').split('/')[0].split(';')[0].split(':')[0];
}

async function getEroamingSipDomain() {
  return (
    normalizeSipDomain(await getSettingValue('astpp_sip_domain')) ||
    normalizeSipDomain(await getSettingValue('user_sip_domain')) ||
    normalizeSipDomain(process.env.ASTPP_SIP_DOMAIN) ||
    normalizeSipDomain(process.env.USER_SIP_DOMAIN) ||
    'sip.esimdata.shop'
  );
}

async function buildEroamingSipForwardDestination(msisdn: unknown) {
  const digits = phoneDigits(msisdn);
  const domain = await getEroamingSipDomain();
  return digits && domain ? `sip:${digits}@${domain}` : '';
}

async function buildAstppInternationalSipDestination(value: unknown, countryCode?: string | null) {
  const destination = normalizeOutboundVoiceDestination(value, countryCode);
  const domain = await getEroamingSipDomain();
  return destination && domain ? `sip:${destination}@${domain};user=phone` : '';
}

function toVonageSipCallbackValue(value: unknown) {
  return trim(value).replace(/^sips?:/i, '').split(';')[0];
}

async function resolveForwardingDestination({
  forwardingType,
  forwardingDestination,
  msisdn,
  countryCode,
}: {
  forwardingType: VirtualNumberForwardingType;
  forwardingDestination?: string | null;
  msisdn?: string | null;
  countryCode?: string | null;
}) {
  if (forwardingType === 'sip') {
    return await buildEroamingSipForwardDestination(msisdn);
  }

  if (forwardingType === 'international') {
    const destination = normalizeOutboundVoiceDestination(forwardingDestination, countryCode);
    return destination || trim(forwardingDestination) || null;
  }
...
export async function buildVonageVoiceAnswerNcco(payload: Record<string, any>, baseUrl: string) {
  ...
  if (fromUser && rawOutboundDestination) {
    ...
    const astppDestination = await buildAstppInternationalSipDestination(
      rawOutboundDestination,
      number.countryCode,
    );
    if (!astppDestination) {
      return voiceUnavailableNcco('The ASTPP international route is not available right now.');
    }
  ...
  if (routingType === 'international' && routingDestination) {
    const destination = await buildAstppInternationalSipDestination(
      routingDestination,
...
export async function handleVonageVoiceEvent(payload: Record<string, any>) {
  ...
  const callId = trim(payload.uuid || payload.call_uuid || payload.callId);
  const conversationId = trim(payload.conversation_uuid || payload.conversationUuid);
  let existingCall: typeof virtualVoiceCalls.$inferSelect | null = null;

  if (callId) {
    [existingCall] = await db
      .select()
      .from(virtualVoiceCalls)
      .where(eq(virtualVoiceCalls.providerCallId, callId))
      .limit(1);
  }

  const existingMetadata = ((existingCall?.metadata as Record<string, any>) || {}) as Record<
    string,
    any
  >;
  const callMetadata = {
  ...
  if (existingCall) {
    await db
      .update(virtualVoiceCalls)
      .set({
        status,
        conversationId: conversationId || existingCall.conversationId,
        metadata: callMetadata,
        updatedAt: new Date(),
      })
      .where(eq(virtualVoiceCalls.id, existingCall.id));
  ...
  const chargeStatuses = new Set(['answered', 'completed']);
  if (
    chargeStatuses.has(status) &&
    existingCall &&
    !(existingMetadata.inboundVoiceCharge || existingMetadata.inboundVoiceChargeError)
  ) {
    const inventoryItem = await findInventoryForVirtualNumber(number);
    try {
      const usageCharge = await chargeVirtualNumberUsage({
        ...
        usageType: 'voice',
        direction: 'inbound',
        description: `Virtual number inbound voice call to ${number.msisdn}`,
        referenceId: `${number.msisdn}:voice:inbound:${callId || conversationId}`,
        metadata: {
          from,
          callId: callId || null,
          conversationId: conversationId || null,
          chargePoint: 'vonage_voice_event',
          route: ((number.metadata as Record<string, any>) || {}).routing || null,
        },
      });
      ...
      await db
        ...
        .set({
          metadata: {
            ...callMetadata,
            inboundVoiceCharge: usageCharge,
            inboundVoiceChargedAt: new Date().toISOString(),
          },
          updatedAt: new Date(),
        })
        .where(eq(virtualVoiceCalls.id, existingCall.id));
      ...
      await db
        ...
        .set({
          metadata: {
            ...callMetadata,
            inboundVoiceChargeError: error?.message || 'Inbound voice charge failed',
            inboundVoiceChargeFailedAt: new Date().toISOString(),
          },
          updatedAt: new Date(),
        })
        .where(eq(virtualVoiceCalls.id, existingCall.id));
  ...
  if (!missedStatuses.has(status)) return;
...
export async function storeVonageVoicemail(payload: Record<string, any>) {
  ...
  return toPublicVirtualVoicemail(voicemail);
}
...
async function updateVonageNumberWebhook(
  ...
  if (voiceRouting?.type && voiceRouting.value) {
    formParams.voiceCallbackType = voiceRouting.type;
    formParams.voiceCallbackValue = voiceRouting.value;
    if (voiceRouting.statusCallback) {
      formParams.voiceStatusCallback = voiceRouting.statusCallback;
    }
    if (voiceRouting.type === 'app' && config.applicationId) {
      formParams.app_id = config.applicationId;
    }
  } else if (config.applicationId) {
    formParams.app_id = config.applicationId;
  }

  const form = buildApiForm(formParams);
...
function voiceStatusCallbackFromSmsWebhook(webhookUrl: string) {
  try {
    return `${new URL(webhookUrl).origin}/api/webhooks/vonage/voice/events`;
  } catch {
    return '';
  }
}

async function buildVonageVoiceRoutingUpdate(
  config: VonageConfig,
  forwardingType: VirtualNumberForwardingType,
  destination: string | null,
  webhookUrl: string,
) {
  const statusCallback = voiceStatusCallbackFromSmsWebhook(webhookUrl);

  if (forwardingType === 'sip' && destination) {
    return {
      type: 'sip' as const,
      value: toVonageSipCallbackValue(destination),
      statusCallback,
    };
  }

  if (forwardingType === 'international' && destination) {
    const sipDestination = await buildAstppInternationalSipDestination(destination);
    if (sipDestination) {
      return {
        type: 'sip' as const,
        value: toVonageSipCallbackValue(sipDestination),
        statusCallback,
      };
    }
  }

  if (forwardingType === 'international' && destination && config.applicationId) {
    return {
      type: 'app' as const,
...
async function syncVonageNumberForwarding(
  config: VonageConfig,
  item: typeof virtualNumberInventory.$inferSelect,
  routing?: { type?: unknown; destination?: unknown } | null,
) {
  if (!config.enabled || !hasCredentials(config) || !config.inboundWebhookUrl) return;

  const forwardingType = normalizeForwardingType(routing?.type, 'none');
  const destination = await resolveForwardingDestination({
    forwardingType,
    forwardingDestination: trim(routing?.destination),
    msisdn: item.msisdn,
  ...
  await updateVonageNumberWebhook(
    ...
    await buildVonageVoiceRoutingUpdate(
      config,
      forwardingType,
      destination,
      config.inboundWebhookUrl,
...
async function fulfillApplicationWithAssignedNumber({
  ...
  const [account] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
  queueAdminAlert('eroaming_order', {
    title: 'New eRoaming Number Assigned',
    message: `${account?.email || userId} now has eRoaming number ${msisdn}.`,
    actionPath: '/admin/virtual-numbers/bought-dids',
    metadata: {
      userId,
      customerEmail: account?.email || null,
      msisdn,
      countryCode,
      virtualNumberId: assignedNumber.id,
      inventoryId: inventoryId || null,
      pricing,
    },
  });
...
export async function getUserVirtualSmsMessages(userId: string) {
  ...
  const messages = await db
    ...
    .limit(100);
  return messages.map(toPublicVirtualSmsMessage);
}
...
export async function createVirtualNumberApplication({
  ...
  const normalizedForwardingType = normalizeForwardingType(forwardingType, 'sip');
  const normalizedReminderDays = Math.max(1, Math.trunc(Number(reminderDays) || 3));
  const normalizedForwardingDestination = await resolveForwardingDestination({
    forwardingType: normalizedForwardingType,
    forwardingDestination,
    msisdn: desiredNumber,
    countryCode,
  });
  ...
  const [account] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
  queueAdminAlert('eroaming_order', {
    title: 'New eRoaming Number Request',
    message: `${account?.email || userId} requested an eRoaming number for ${countryCode}.`,
    actionPath: '/admin/virtual-numbers/bought-dids',
    metadata: {
      userId,
      customerEmail: account?.email || null,
      ...
      countryCode,
      desiredNumber: trim(desiredNumber) || null,
      inventoryId: trim(inventoryId) || null,
  ...
  if (selectedInventoryId) {
    ...
    const selectedForwardingDestination = await resolveForwardingDestination({
      forwardingType: normalizedForwardingType,
      forwardingDestination,
      msisdn: selectedInventory.msisdn,
      countryCode: selectedInventory.countryCode,
    });
    ...
    await db
      ...
      .set({
        ...
        metadata: {
          ...
          reminderDays: normalizedReminderDays,
          forwardingType: normalizedForwardingType,
          forwardingDestination: selectedForwardingDestination,
  ...
  return {
    application: toPublicVirtualNumberApplication(application),
    assignedNumber: null,
...
async function sendUserVirtualSmsWithNumber(
  ...
  const inventoryItem = await findInventoryForVirtualNumber(number);
  const numberMetadata = (number.metadata as Record<string, any>) || {};
  const sender = await resolveSmsSenderForUser(userId, number, selectedSenderId);
  const referenceId = `${number.msisdn}:sms:outbound:${Date.now()}`;
  await assertOwnSmsCarrierConfigured();
  const fromNumber = trim(sender);
  const outboundTo = trim(to);
  ...
  const usageCharge = await chargeVirtualNumberUsage({
    ...
    metadata: {
      ...
      senderId: getSmsSenderIdMetadata(numberMetadata),
      requestedFrom: number.msisdn,
      submittedFrom: fromNumber,
    },
  });

  const response = await sendSmsWithOwnCarrier({
    from: fromNumber,
    to: outboundTo,
    text,
    referenceId,
  });
  const status = response.status === 'queued' ? 'queued' : 'sent';
  const actualSubmittedFrom = trim(response.submittedFrom) || fromNumber;
  ...
  const [sms] = await db
    ...
    .values({
      ...
      direction: 'outbound',
      fromNumber: actualSubmittedFrom,
      toNumber: outboundTo,
      text,
      ...
      metadata: {
        ...response,
        requestedTo: to,
        requestedSubmittedFrom: fromNumber,
        submittedFrom: actualSubmittedFrom,
        submittedTo: outboundTo,
        senderId: getSmsSenderIdMetadata(numberMetadata),
        billing: usageCharge,
      },
    })
  ...
  return toPublicVirtualSmsMessage(sms);
}

export async function sendUserVirtualSms(
  userId: string,
  to: string,
  text: string,
  selectedSenderId?: string | null,
  ...
  return sendUserVirtualSmsWithNumber(userId, number, to, text, selectedSenderId);
}
...
export async function requestUserVirtualNumberSenderId(
  userId: string,
  virtualNumberId: string,
  ...
  const normalizedSenderId = normalizeSmsSenderId(requestedSenderId);
  await assertSenderIdLimitAvailable(userId, normalizedSenderId);

  const metadata = (number.metadata as Record<string, any>) || {};
  const currentSenderId = getSmsSenderIdMetadata(metadata);
  const approvedList = currentSenderId.approvedList.length
    ? currentSenderId.approvedList
    : currentSenderId.approved
      ? [currentSenderId.approved]
      : [];
  const nextSenderId = {
    ...currentSenderId,
    approvedList,
    requested: normalizedSenderId,
    status: 'pending',
  ...
  const [account] = await db.select().from(users).where(eq(users.id, userId)).limit(1);
  queueAdminAlert('sender_id_approval', {
    title: 'Sender ID Approval Required',
    message: `${account?.email || userId} requested Sender ID ${normalizedSenderId} for eRoaming number ${number.msisdn}.`,
    actionPath: '/admin/virtual-numbers/bought-dids',
    metadata: {
      userId,
      customerEmail: account?.email || null,
      ...
      msisdn: number.msisdn,
      requestedSenderId: normalizedSenderId,
      previousApprovedSenderId: currentSenderId.approved || null,
    },
  });

  return toPublicVirtualNumber(updated);
}

export async function reviewVirtualNumberSenderIdFromInventory(
  inventoryId: string,
  ...
  if (!linkedNumber) throw new Error('Assign this eRoaming number before reviewing Sender ID');

  const metadata = (linkedNumber.metadata as Record<string, any>) || {};
  const currentSenderId = getSmsSenderIdMetadata(metadata);
  ...
  const nextSenderId =
    action === 'approve'
      ? (() => {
          const approved = normalizeSmsSenderId(options.senderId || currentSenderId.requested || '');
          const approvedList = Array.from(
            new Map(
              [...currentSenderId.approvedList, currentSenderId.approved, approved]
                .filter(Boolean)
                .map((sender) => [String(sender).toLowerCase(), String(sender)]),
            ).values(),
          );
          return {
            ...currentSenderId,
      ...
      : {
          ...
          status: 'rejected',
          reviewedAt,
          rejectedReason: trim(options.rejectionReason) || 'Rejected by admin',
        };
...
export async function storeInboundVonageSms(payload: Record<string, any>) {
  await ensureVonageSchema();
  const to = trim(payload.to || payload.recipient || payload.number || payload['to-number']);
  const toLookup = normalizeSmsAddressForLookup(to);
  const from = trim(payload.msisdn || payload.from || payload.sender || payload['from-number']);
  const text = trim(payload.text);
  const providerMessageId = getInboundSmsMessageId(payload);
  ...
  const [number] = await db
    ...
    .where(
      and(
        eq(userVirtualNumbers.status, 'active'),
        sql`regexp_replace(${userVirtualNumbers.msisdn}, '[^0-9]', '', 'g') = ${toLookup}`,
      ),
    )
    .limit(1);
  ...
  const usageCharge = await chargeVirtualNumberUsage({
    ...
    referenceId: `${number.msisdn}:sms:inbound:${providerMessageId || Date.now()}`,
    metadata: {
      from,
      textLength: text.length,
      providerMessageId,
...
export async function syncAdminVonageOwnedNumbers(options?: {
  ...
  for (const ownedNumber of Array.from(ownedByMsisdn.values())) {
    ...
    if (options?.updateWebhooks && config.inboundWebhookUrl) {
      try {
        if (inventoryItem) {
          await syncVonageNumberForwarding(
            config,
            inventoryItem,
            ((inventoryItem.metadata as Record<string, any>) || {}).routing,
          );
        } else {
          await updateVonageNumberWebhook(
            config,
...
export async function buyAdminVirtualNumberInventory({
  ...
  const forwardingType = normalizeForwardingType(
    pricingInput.forwardingType,
    assignedUserId ? 'sip' : 'none',
  );
  const forwardingDestination = await resolveForwardingDestination({
    forwardingType,
    forwardingDestination: pricingInput.forwardingDestination,
  ...
  await syncVonageNumberForwarding(config, inventoryItem, {
    type: forwardingType,
    destination: forwardingDestination,
  });
...
export async function updateVirtualNumberInventoryItem(
  ...
  const nextForwardingType =
    updates.forwardingType !== undefined
      ? normalizeForwardingType(updates.forwardingType, 'none')
      : shouldDefaultAssignedNumberToSip
        ? 'sip'
        : normalizeForwardingType(currentRoutingType, 'none');
  ...
  const nextForwardingDestination = await resolveForwardingDestination({
    forwardingType: nextForwardingType,
    forwardingDestination: requestedForwardingDestination,
    msisdn: currentItem.msisdn,
  ...
  const syncForwarding = async (item: typeof virtualNumberInventory.$inferSelect) => {
    const config = await getVonageConfig();
    await syncVonageNumberForwarding(config, item, linkedNumberMetadata.routing);
  };
  ...
  await syncForwarding(updatedItem);
  return updatedItem;
}
...
export async function getVonageUserDashboard(userId: string) {
  await processVirtualNumberSubscriptionLifecycle();
  const [config, number, numbers, application, messages, account, sipAccount, senderIdLimit] = await Promise.all([
    getVonageConfig(),
  ...
  return {
    ...
    sipAccount,
    number: number ? toPublicVirtualNumber(number, billingByNumberId.get(number.id) || null) : null,
    numbers: numbers.map((item) => toPublicVirtualNumber(item, billingByNumberId.get(item.id) || null)),
    application,
    messages,
    senderIdLimit,
    smsSenderOptions: getSmsSenderOptionsForNumbers(numbers, number),
  };
}
...
function toPublicVirtualNumber(
  ...
  return {
    ...
    senderId: getSmsSenderIdMetadata(metadata),
...
export async function getUserVirtualNumberWorkspace(userId: string, virtualNumberId?: string) {
  await processVirtualNumberSubscriptionLifecycle();
  const [config, numbers, account, sipAccount, senderIdLimit] = await Promise.all([
    getVonageConfig(),
    getUserVirtualNumbers(userId),
    db
      .select()
        ...
        db
          ...
          .where(
            and(
              ...
              sql`${virtualVoicemails.status} <> 'deleted'`,
            ),
          )
          .orderBy(desc(virtualVoicemails.createdAt))
          .limit(100),
  ...
  const publicMessages = messages.map(toPublicVirtualSmsMessage);
  const publicVoiceCalls = voiceCalls.map(toPublicVirtualVoiceCall);
  const publicVoicemails = voicemails.map(toPublicVirtualVoicemail);
  ...
  return {
    ...
    receivingMessages: publicMessages.filter((message) => message.direction === 'inbound'),
    sendingMessages: publicMessages.filter((message) => message.direction === 'outbound'),
    voiceCalls: publicVoiceCalls,
    voicemails: publicVoicemails,
    senderIdLimit,
    smsSenderOptions: getSmsSenderOptionsForNumbers(numbers, selectedNumber),
  };
}
...
export async function getUserVirtualNumberVoicemails(userId: string, virtualNumberId: string) {
  ...
  const voicemails = await db
    ...
    .orderBy(desc(virtualVoicemails.createdAt))
    .limit(100);
  return voicemails.map(toPublicVirtualVoicemail);
}

export async function deleteUserVirtualNumberVoicemail(
  userId: string,
  virtualNumberId: string,
  voicemailId: string,
) {
  ...
  const [voicemail] = await db
    ...
    .where(
      and(
        eq(virtualVoicemails.id, voicemailId),
        eq(virtualVoicemails.userId, userId),
        eq(virtualVoicemails.virtualNumberId, number.id),
      ),
    )
  ...
  if (!voicemail) {
    throw new Error('Voice Mail was not found');
  }
