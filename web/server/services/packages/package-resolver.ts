...
export async function getProviderSpecificPackageId(
  ...
  } else if (providerPackageTable === 'airhub_packages') {
    const { airhubPackages } = await import("@shared/schema");
    result = await db.query.airhubPackages.findFirst({
      where: eq(airhubPackages.id, providerPackageId),
    });
    return result?.airhubId || null;
  }
