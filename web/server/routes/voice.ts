...
import { quoteVoiceCall } from "server/services/voice-rating-service";
import {
...
router.post("/rate-quote", requireAuth, async (req: any, res) => {
  try {
    const destination = req.body?.destination || req.body?.to || req.body?.referenceNumber || "";
    const data = await quoteVoiceCall(req.userId, destination);
    return ApiResponse.success(res, "Voice rate quote loaded successfully", data);
  } catch (error: any) {
    return ApiResponse.badRequest(res, error.message || "Failed to load voice rate quote");
  }
});
