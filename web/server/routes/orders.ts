...
function publicPackage(pkg: any) {
  if (!pkg) return pkg;
  const { airaloPrice, wholesalePrice, ...safePackage } = pkg;
  return safePackage;
}
...
function publicOrderStatus(order: any) {
  const status = String(order?.status || "").toLowerCase();
  if ((status === "completed" || status === "active" || status === "in_use") && !hasProviderFulfillment(order)) {
    return "processing";
  }
    ...
    const ordersWithDetails = await Promise.all(
      userOrders.map(async (order) => {
        ...
        return {
          ...
          fulfillmentStatus: hasProviderFulfillment(order) ? "provisioned" : "awaiting_provider",
    ...
    if (orderType === "single") {
      ...
      try {
        ...
        queueAdminAlert("esim_order", {
          ...
          message: `eSIM order for ${user?.email || userId} completed.`,
          ...
          metadata: {
            ...
            validity: pkg.validity,
            price: pkg.price,
            providerOrderId: orderDetails.airaloOrderId,
            source: "web_order",
          },
        });
      ...
      try {
        ...
        queueAdminAlert("esim_order", {
          title: "New eSIM Data Order",
          message: `Batch eSIM order for ${user?.email || userId} is processing (${quantity} eSIMs).`,
          actionPath: "/admin/orders",
          metadata: {
