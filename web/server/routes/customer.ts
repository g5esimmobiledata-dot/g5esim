...
function buildLpaCode(order: any) {
  const smdpAddress = firstString(order?.smdpAddress, order?.smdp_address);
  const activationCode = firstString(order?.activationCode, order?.activation_code);
  if (!smdpAddress || !activationCode) return null;
  return `LPA:1$${smdpAddress}$${activationCode}`;
}

async function publicOrderWithQr(order: any) {
  ...
  const qrPayload = manualCode?.startsWith("1$") ? `LPA:${manualCode}` : manualCode;

  if (qrPayload?.startsWith("LPA:")) {
  ...
  return {
    ...
    hasQrCode: Boolean(qrCodeImage || manualCode),
  };
}
  ...
  try {
    ...
    const ordersWithDetails = await Promise.all(
      orders.map(async (order) => {
        ...
        return { ...(await publicOrderWithQr(order)), package: { ...publicPackage(pkg), destination } };
      })
    );
...
router.get("/esims", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const orders = await storage.getOrdersByUser(userId);
    const esims = await Promise.all(orders
      .filter(order => order.iccid && order.status === "completed")
      .map(publicOrderWithQr));
