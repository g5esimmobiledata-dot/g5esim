...
router.get("/", requireAdmin, async (req, res) => {
  try {
    const { status, priority, search, channel, page = "1", limit = "25" } = req.query;
    ...
    if (channel === "concierge") {
      conditions.push(eq(tickets.title, "AI Voice Concierge Request"));
      if (!status) {
...
router.get("/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const messages = replies.map((r) => ({
      ...
      senderType: r.adminId ? "admin" : r.userId ? "user" : "ai",
      senderName: r.adminId ? "Support" : r.userId ? "User" : "Concierge AI",
      message: r.message,
...
router.put("/:id", requireAdmin, async (req, res) => {
  try {
    ...
    const validPriorities = ["low", "medium", "high", "urgent", "normal"];

    const updateData: any = { updatedAt: new Date() };

    if (status !== undefined) {
      if (!validStatuses.includes(String(status))) {
        return ApiResponse.badRequest(res, "Invalid ticket status");
      }
    ...
    if (priority !== undefined) {
      if (!validPriorities.includes(String(priority))) {
        return ApiResponse.badRequest(res, "Invalid ticket priority");
      }
...
router.post("/:id/messages", requireAdmin, async (req, res) => {
  try {
    ...
    if (!isInternal && ticket.userId) {
      ...
      const title = `Update on Your Ticket #${id}`;
      const body = message.trim().substring(0, 100);
      const data = {
        ticketId: id,
        replyId: reply.id,
        type: "ticket_reply",
      };
      const oneSignalResult = user
        ? await sendOneSignalPushToUsers([user.id], title, body, data).catch((error) => {
            console.warn("OneSignal ticket push unavailable:", error instanceof Error ? error.message : error);
            return null;
          })
        : null;
      ...
      if (fcmToken) {
        ...
        try {
          if (oneSignalResult?.sent) {
            console.log("OneSignal push sent to user:", user.id);
          } else {
...
router.post("/:id/messagesOld", requireAdmin, async (req, res) => {
  try {
    ...
    if (!isInternal && ticket.userId) {
      ...
      const fcmToken = user?.fcmToken;
      const title = "New message on Ticket";
      const body = message.trim().substring(0, 100);
      const data = {
        ticketId: id,
        replyId: reply.id,
      ...
      if (fcmToken) {
        ...
        try {
          if (oneSignalResult?.sent) {
            console.log("OneSignal notification sent to user:", user.id);
          } else {
          const messaging = await getAdminMessaging();
          await messaging.send(payload);
          }
