...
import { sendOneSignalEmailToRecipients } from "../../services/onesignal-service";
...
async function sendNotificationEmails(
  ...
  try {
    ...
    const oneSignalResult = await sendOneSignalEmailToRecipients(
      recipients.map((recipient) => ({ id: recipient.id, email: recipient.email })),
    ...
    if (oneSignalResult) {
      return {
        emailsSent: oneSignalResult.sent,
        emailsFailed: oneSignalResult.failed,
      };
    }
  } catch (error) {
    console.warn("OneSignal notification email failed, falling back to SMTP:", error instanceof Error ? error.message : error);
  }
  ...
  try {
    ...
    const unionSql = `
      WITH items AS (
        ...
        SELECT
          id,
          'admin_alert'::text AS source,
          COALESCE(metadata->>'event', type)::text AS type,
          COALESCE(metadata->>'iccid', '')::text AS iccid,
          true AS processed,
          COALESCE(NULLIF(metadata->>'emailsSent', ''), '0')::int > 0 AS "emailSent",
          NULLIF(metadata->>'emailError', '') AS error,
          jsonb_build_object(
            'title', title,
            'message', message,
            'metadata', metadata
          ...
          created_at AS "createdAt"
        FROM notifications
        WHERE metadata->>'source' = 'admin_alert'
      )
    `;
