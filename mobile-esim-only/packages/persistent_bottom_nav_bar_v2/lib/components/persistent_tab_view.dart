...
class _PersistentTabViewState extends State<PersistentTabView> {
  ...
  bool _isInvalidTabIndex(int index) =>
      index < 0 ||
      index >= widget.tabs.length ||
      widget.tabs[index].onPressed != null;

  int _fallbackTabIndex({int? preferredIndex}) {
    if (widget.tabs.isEmpty) {
      return 0;
    }

    final preferred = preferredIndex ?? _controller.initialIndex;
    if (preferred >= 0 &&
        preferred < widget.tabs.length &&
        widget.tabs[preferred].onPressed == null) {
      return preferred;
    }

    final firstEnabledIndex = widget.tabs.indexWhere(
      (tab) => tab.onPressed == null,
    );
  ...
  void initState() {
    ...
    if (widget.tabs.isNotEmpty && _isInvalidTabIndex(_controller.index)) {
      _controller.jumpToTab(_fallbackTabIndex());
    }
  ...
  void didUpdateWidget(covariant PersistentTabView oldWidget) {
    ...
    // If tabs change while modules are hidden/shown, the controller can still
    // point at an index from the previous tab list. Clamp before indexing.
    if (widget.tabs.isNotEmpty && _isInvalidTabIndex(_controller.index)) {
      final newIndex = _fallbackTabIndex();
  ...
  NavigatorState? _currentNavigatorState() {
    if (_controller.index < 0 || _controller.index >= _navigatorKeys.length) {
      return null;
    }

    return _navigatorKeys[_controller.index].currentState;
  }
