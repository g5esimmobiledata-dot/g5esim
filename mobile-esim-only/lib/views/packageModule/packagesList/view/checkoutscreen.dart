...
class _CheckoutscreenState extends State<Checkoutscreen> {
  ...
  String _gatewayProvider(GatewayItem gateway) {
    return _normalizeProvider(gateway.provider ?? gateway.id ?? '');
  }
  ...
  bool _isCryptoProvider(String provider) {
    final normalized = _normalizeProvider(provider);
    return normalized == 'nowpayments' ||
        normalized == 'cryptomus' ||
        normalized == 'directusdt';
  }

  String _cryptoProviderTitle(dynamic provider) {
  ...
  bool _gatewayRequiresLogin(GatewayItem gateway) {
    ...
    return _isCryptoProvider(provider);
  }
  ...
  Future<void> _confirmExternalPayment(Payment payment) async {
    final provider = _normalizeProvider(payment.provider ?? '');
  ...
  void _showCryptoPaymentSheet(Payment payment) {
    final providerName = _cryptoProviderTitle(payment.provider);
              ...
              listener: (context, state) async {
                if (state is ApiLoading) {
                  ...
                  if (state.data.success == true) {
                    ...
                    if (payment == null || provider.isEmpty) {
                      global.showToastMessage(
                    ...
                    } else if (_isCryptoProvider(provider)) {
                      final url = _paymentRedirectUrl(payment);
                      if (url != null) {
                        await _openAndConfirmExternalPayment(
                          payment: payment,
